...
class WolfTagBrief {
	...
	private function filter($string) {
		...
		// treat w:... as wolf tag and replace it with a link
		$string = preg_replace('/w:(\w+)/', '<a href="'.$_SERVER['REQUEST_URI'].'#tag_$1">$1</a>', $string);
		return $string;
	}
...
class WolfTagsDocumentation {
	...
	public function __construct() {
		...
		usort($this->briefs, 'WolfTagBrief::compare');
	}
	 ...
	 * parsed function belongs to a class derived from WolfTags. But it checks
	 * if the function belongs to a tag in the tag list. So there should only
	 * be problems if there is another function with the same name.
	 */
	protected function parse_source($string) {
		$definedTags = WolfTagsList::get();
		$this->briefnum = 0;
		...
		foreach($matches as $match) {
			...
			if (isset($definedTags[$name]))
				array_push($this->briefs, new WolfTagBrief($name, $brief));
		}
	}
