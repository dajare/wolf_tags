...
include_once('WolfTags.php');
include_once('WolfTagsList.php');
include_once('WolfTagsParserPreprocessor.php');

class WolfTagsParser {

	protected $page, $tags;

	/**
	 * Creates a new WolfTagParser object for $page.
	 */
	public function __construct($page) {
		$this->page = $page;
		$this->tags = WolfTagsList::get();
	}
	...
	public function parse($string, &$parent = NULL, $defaultArgs = array()) {
		...
		// Preprocessing
		$preprocessor = new WolfTagsParserPreprocessor();
		$string = $preprocessor->run($string);

		// Parse top-level tags. Top-level tags have been marked with sign '@'
		// by the preprocessor.
		preg_match_all("|<w:@(\w+?)([^<>]*)>(.*)</w:@\\1\s*>|Us", $string, $matches, PREG_SET_ORDER);
		foreach($matches as $match) {
			$reference = $match[0];
			$name      = $match[1];
			$content   = $match[3];
			...
			try {
				if (isset($this->tags[$name])) {
					...
					throw new Exception("Tag \"&lt;w:$name /&gt;\" wasn't defined!");
				}
			}
			catch (Exception $e) {
