...
Observer::observe('page_found', 'WolfTagsHacks::infiltrate');

Observer::stopObserving('page_not_found', 'behavior_page_not_found');
Observer::observe('page_not_found', 'WolfTagsHacks::page_not_found_hack');

class WolfTagsHacks {

	/**
	 * Function to be called by the observer. This function will call the main
	 * wolf tags parser on the content of $page.
	 */
	public static function infiltrate($page) {
		...
		foreach($observerList as $callback) {
			if ($callback == 'WolfTagsHacks::infiltrate')
				$alreadyCalled = false;
	...
	public static function get_page_layout($page) {
		if (ALLOW_PHP) {
			...
			$statement = $__CMS_CONN__->prepare($query);
	...
	public static function get_page_content($page, $part, $inherit) {
		...
		if ($statement = $__CMS_CONN__->prepare($query)) {
			do {
	...
	public static function page_not_found_hack() {
		...
		$alreadyCalled = true;
		foreach($observerList as $callback) {
			if ($callback == 'WolfTagsHacks::page_not_found_hack')
				$alreadyCalled = false;
			elseif (!$alreadyCalled)
