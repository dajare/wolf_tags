...
Wolf tags are very similar to HTML tags, or even XML tags. With _WolfTags Plugin_ plugin enabled, you can use some standard WolfTags and first of all easily create own WolfTags.

_WolfTags Plugin_ will add the same functionality to Wolf CMS as offered by "Radius tags":http://wiki.github.com/radiant/radiant/radius-tags in Radinat CMS (but with a preceding 'w:' instead of 'r:').

WolfTags may be used in a Page, Snippet, or Layout.

h2. Configuration

To allow execution of PHP code as common in Wolf CMS add the line:

<pre>
define(ALLOW_PHP, true);
</pre>

to @WOLF_ROOT/config.php@. (As this file is normally not writable you have to give yourself write permission to that file first!)
...
WolfTags can be included on pages using either the Markdown or Textile. (And usage with Textile does not require any "notextile." markup.)
...
Like HTML tags, Wolf tags can have some content as well. For example:

<pre>
<w:uppercase>Welcome to page <title /></w:uppercase>
</pre>
